        ...
        stage ('Sonar Analysis') {
            ...
            steps {
                withSonarQubeEnv('SonarQubeLocal') {
                    bat "${scannerHome}/bin/sonar-scanner -e -Dsonar.projectKey=DeployBackend -Dsonar.projectBaseDir=. -Dsonar.host.url=http://localhost:9000 -Dsonar.login=sqp_ac13a96a4d06bfe2cf6d77c613ca42a7b0250cb1 -Dsonar.java.binaries=target -Dsonar.sources=src/main/java -Dsonar.tests=src/test/java -Dsonar.test.inclusions=src/test/java/br/ce/wcaquino/taskbackend/utils/DateUtilsTest.java -Dsonar.test.inclusions=src/test/java/br/ce/wcaquino/taskbackend/controller/TaskControllerTest.java -Dsonar.java.test.libraries=target/tasks-backend/WEB-INF/lib -Dsonar.junit.reportsPath=target/surefire-reports/TEST-*.xml -Dsonar.java.coveragePlugin=jacoco -Dsonar.coverage.jacoco.reportPath=target/site/jacoco/jacoco.xml -Dsonar.java.libraries=target/tasks-backend/WEB-INF/lib -Dsonar.qualitygate.wait=true -Dsonar.coverage.exclusions=**TaskBackendApplication.java**,**/model/**"
                }                
            }
        }
        stage ('Quality Gate') {
